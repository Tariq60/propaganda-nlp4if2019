...
from output import sentences_to_pred_output, soft_f1
from tqdm import *
...
class ModelTrainer:
    ...
    def __init__(self,
                 ...
                 id_dict=None, gold_train_dict=None, gold_dev_dict=None, gold_test_dict=None,
        ...
        self.gold_train_dict = gold_train_dict
        self.gold_dev_dict = gold_dev_dict
        self.gold_test_dict = gold_test_dict
    ...
    def train(self,
        ...
        # if training also uses test data, include in training set
        if train_with_dev:
            ...
            for epoch in range(0 + self.epoch, max_epochs + self.epoch):
                ...
                if monitor_train:
                    train_metric, train_loss = self._calculate_evaluation_results_for(
                        'TRAIN', self.corpus.train, evaluation_metric, embeddings_in_memory, eval_mini_batch_size, id_dict=self.id_dict['train'],
                        gold_dict = self.gold_train_dict )

                if not train_with_dev:
                    dev_metric, dev_loss = self._calculate_evaluation_results_for(
                        'DEV', self.corpus.dev, evaluation_metric, embeddings_in_memory, eval_mini_batch_size, id_dict=self.id_dict['test'],
                            gold_dict = self.gold_dev_dict )
                ...
                if not param_selection_mode:
                    test_metric, test_loss = self._calculate_evaluation_results_for(
                        ...
                        base_path / 'test.tsv',  id_dict=self.id_dict['test'], gold_dict = self.gold_dev_dict )
                ...
                # anneal against train loss if training with test, otherwise anneal against test score
                ...
                # if we use test data, remember best model based on test evaluation score
                if not train_with_dev and not param_selection_mode and current_score == scheduler.best:
                    self.model.save(base_path / 'best-model.pt')

            # if we do not use test data for model selection, save final model
    ...
    def _calculate_evaluation_results_for(self,
        ...
        metric, loss = ModelTrainer.evaluate(self.model, dataset, eval_mini_batch_size=eval_mini_batch_size,
                                             embeddings_in_memory=embeddings_in_memory, out_path=out_path, id_dict=id_dict,
                                          gold_dict=gold_dict)
    ...
    def evaluate(model: flair.nn.Model, data_set: List[Sentence],
                 ...
                 embeddings_in_memory: bool = True,
                 out_path: Path = None, id_dict=None,
                                          gold_dict=None) -> (
            dict, float):
        if isinstance(model, TextClassifier):
            ...
            return ModelTrainer._evaluate_sequence_tagger(model, data_set, eval_mini_batch_size, embeddings_in_memory,
                                                          out_path, id_dict=id_dict,
                                          gold_dict=gold_dict)



    @staticmethod
    def _evaluate_sequence_tagger_to_file(model,
                                  sentences: List[Sentence],
                                  eval_mini_batch_size: int = 32,
                                  embeddings_in_memory: bool = True,
                                  out_path: Path = None, id_dict=None,
                                          gold_dict=None) -> (dict, float):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(sentences):
                ...
                for (sentence, sent_tags) in zip(batch, tags):
                    ...
                    pred_dict = sentences_to_pred_output(sentence, id_dict[sentence_counter]['d_id'], pred_dict, predictions=sent_tags)
            ...
            # soft_f1_score = soft_f1(pred_dict, gold_dict)

            # print("soft_f1_score: " + str(soft_f1_score))
            ...
            with open('prediction_train.txt', 'w') as f:
                for key, list in pred_dict.items():
                    for start, end_token, prediction in list:
                        if prediction != 'O':
                            f.write(str(key) + '\t' + str(prediction) + '\t' + str(start) + '\t' + str(end_token) + '\n' )
